...
def test_order_saved_with_location_id(tmp_path: Path, monkeypatch) -> None:
    ...
    setup_session: Session = testing_session_local()
    try:
        location = Location(company_name="Beta", address="Street 2", is_active=True)
...
def test_catering_orders_summary_groups_by_location(tmp_path: Path, monkeypatch) -> None:
    ...
    try:
        ...
        order_a = Order(user_id=1, location_id=location_a.id, order_date=date.today(), status="created")
        order_b = Order(user_id=2, location_id=location_b.id, order_date=date.today(), status="created")
